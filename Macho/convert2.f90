Program convert
  ...
  call execute_command_line('cp ../DATA/Ptot.dat ./DATA_mco/Ptot.dat', exitstat=istat)

  call execute_command_line('cp ../DATA/time.dat ./DATA_mco/time.dat', exitstat=istat)
  ...
  do while(flag_read.eq.0)
     ...
     !
      ...
      open(12,file='./DATA_mco/'//name(1:ic-1)//'.mco')

      !open(13,file='./DATA_mco/'//name(1:ic-1)//'_1D.dat')
   ...
   !   write(12,*) nx/every,ny/every
     ...
     do iy=ny+1,1,-1*every
         ...
         do ix=1,nx+1
            ...
            if(flag.eq.0) write(12,*) ni(ix,iy,1)
