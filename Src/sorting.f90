...
!     COMMENTS:      Sort particles. Particles in the same cell have
!                    neighbors indexes in the array.
!     --------------------------------------------------------------
  implicit none
  integer:: nmax,ntype
  integer:: n(3),i,j,k,ix,iy,ic,ptype,iproc,nproc,pl_max
  ! Particle arrays
  ...
  do i=1,np_tot(ptype,iproc),1
     ...
     sorting(3,i,iproc)= vxp(3,i,ptype,iproc)
  ...
  do i=1,np_tot(ptype,iproc),1
    ...
    ! Save velocity & position
     ...
     vxp(3,j,ptype,iproc)= sorting(3,i,iproc)
...
!     --------------------------------------------------------------
  implicit none
  integer:: nmax,nproc,n(3),i,j,k,ix,iy,ic,iproc,ip,ntype,pl_max, &
  ...
  np_tot_tmp= INT( real(sum_np_tot(ptype))/real(nproc) ) + 1

  !
  ! Count the number of particles in each cell
  !
  !$OMP PARALLEL PRIVATE(iproc,i,ip,xp_new,yp_new,zp_new,&
  !$OMP  vxp_new,vyp_new,vzp_new,ix,iy,ic)
  !$OMP DO
  do iproc=1,nproc
