...
!     -------------------------------------------------------------------
  ...
  integer :: iprint
  integer mpi_rank
  integer:: ipt,i,j,jmax,npt_mx,icol,ncol_mx,sig_npt(ncol_mx), &
  ...
  real(kind=8):: sig_Er(npt_mx),sig(npt_mx,ncol_mx),sig_Eex(ncol_mx,2), &
       ...
       sigv_mx(npart,ncol_mx),Ps,Ek_max
...
200  read(10,FMT='(A6)',END=101,ERR=100,ADVANCE='NO') tname(1)
     ...
     if(tname(1)(1:2).eq.'--') then 
        ...
        if( pname(ntype).eq.'[H-]' .or. pname(ntype).eq.'[D-]' ) tag_neg= ntype
        if( pname(ntype).eq.'[H]' .or. pname(ntype).eq.'[D]' ) tag_neu= ntype
        if( pname(ntype).eq.'[Hb]' .or. pname(ntype).eq.'[Db]' ) tag_b= ntype
...
500              format(' Warning: unknown particle in reaction #',1x,i2)
     ...
     if(mpi_rank.eq.0) write(11,'(i4,1x,a30,1x,f8.2)') icol,rlabel,sig_Eex(icol,ind_Eth) ! write reaction on file 
     ...
     do ipt=1,npt_mx
        ...
        ! Find largest energy in cross-section
        Ek_max= MAX(Ek_max,sig_tmp(ipt,icol,1))
...
!     --------------------------------------------------------------
  ...
  do icol=1,ncol
     ...
     do ipt=1,sig_npt_mx
        ...
        if( sig_L.eq.0.d0 .and. sig_R.gt.0.d0) then 
           if(Eth.gt.Ek_R) then
              print'(" Eth greater than threshold energy in reaction #",1x,i4)',icol
              print*, 'Please correct...'
              call stop_calculation
           endif
        endif
  ...
  do icol=1,ncol
     ...
     do ipt=1,sig_npt_mx
        ...
        if(sig_Er(ipt).ge.0.1d0) & ! Cut off at 0.1 eV
  ...
  if(mpi_rank.eq.0) then
     do ptype=1,ntype
        write (pnum,'(i2)'),ptype
        open(10,file='DATA/reactions.'//adjustl(pnum))
